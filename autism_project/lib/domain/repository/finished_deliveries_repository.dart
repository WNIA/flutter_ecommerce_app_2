...
abstract class FinishedDeliveriesRepository {
  Future<Either<Failure, List>>
      getFinishedDeliveriesRepository(int page, String token);
}
...
class FinishedDeliveriesRepositoryImpl implements FinishedDeliveriesRepository {
  ...
  @override
  Future<Either<Failure, List>> getFinishedDeliveriesRepository(int page, String token) async {
    if(await networkInfo.isConnected) {
      try{
        final remoteData = await remoteDataSource.fetchFinishedDeliveriesPaginationRemote(page, token);
